...
class SynthFromLabelTransform(cc.Transform):
    ...
    def __init__(
        ...
        self.gmm = (
            RandomGaussianMixtureTransform(fwhm=gmm_fwhm, background=0)
            if skip_gmm is not True
            else None
        )
    ...
    def apply_transform(self, lab, parameters=None, coreg=None):
        ...
        if self.gmm is not None:
            img = torch.cat(
                [
                    self.intensity(self.gmm.apply_transform(lab, parameters["gmm"][i]))
        ...
        if coreg is not None:
            if not self.no_augs:
                if isinstance(coreg, (list, tuple)):
                    coreg = [
                        self.deform.apply_transform(cor, parameters["deform"])
                        for cor in coreg
                    ]
                else:
...
class CCSynthSeg:
    ...
    def __call__(self, data):
        d = dict(data)
        d[self.image_key], d[self.label_key], coreg = self.transform(
            d[self.label_key],
            (
                [d[key] for key in self.coreg_keys]
        ...
        if coreg is not None:
            for i, key in enumerate(self.coreg_keys):
                d[key] = coreg[i]
        d["mpm"] = torch.zeros_like(d[self.image_key])
        return d