...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        assert self.upsample in [
            ...
            "transpose",
            "interp",
        ], "upsample must be one of 'subpixel', 'transpose', or 'interp'"
    ...
    def _get_up_layer(
        ...
        up.dimensions = self.dimensions
        up.in_channels = in_channels
        up.out_channels = out_channels
        up.scale_factor = strides
        up.kernel_size = self.up_kernel_size
        up.mode = up_mode
        up.bias = self.bias

        conv = Convolution(  # extra conv post-upsample
            self.dimensions,
            out_channels,
            out_channels,
            strides=1,
