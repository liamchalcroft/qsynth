...
def erode(label):
    return (
        ...
        - torch.nn.functional.conv3d(
            1.0 - label[None],
            torch.ones((1, 1, 3, 3, 3), dtype=label.dtype, device=label.device)
            / (3**3),
            padding=0,
        )[0]
    )
    ...
    def __init__(
        ...
        self.load = Compose(
            transforms=[
                ...
                SpacingD(keys=["label"], pixdim=1 if not lowres else 2),
            ]
        )
        ...
        for i, key in enumerate(self.key_iterator(d)):
            ...
            if self.fs_healthy:
                ...
                if "healthy_meta_dict" in list(d.keys()):
                    d[self.new_keys[i] + "_meta_dict"] = deepcopy(
                        d["healthy_meta_dict"]
                    )
...
class VAEPasteD(MapTransform):
    ...
    def __init__(
        ...
        keys: KeysCollection,
        new_keys: str = "seg",
        model_path: str = "./last_model_jit.pth",
        smooth_lesion=True,
        fill_lesion=False,
        allow_missing_keys=False,
        fs_healthy=False,
    ...
    def __call__(
        ...
        for i, key in enumerate(self.key_iterator(d)):
            self.model.to(d[key].device)
            lesion = self.model(torch.randn(1, 32).to(d[key].device))[0]
            if self.fill:
                lesion = mask_fill(lesion)
            lesion = ((lesion > 0.5) * (lesion < 1)).int()
            lesion_mask = softcp(lesion.float()) if self.smooth else lesion
            if self.lesion_fade is not None:
                lesion_mask = self.lesion_fade(lesion_mask)
                d[key] = paste_lesion(d[key], lesion_mask)
            ...
            if self.fs_healthy:
                ...
                brain_tissue = (
                    (
                        brain_tissue.sum(0, keepdim=True)
                        * GaussianSmooth(3)((d["healthy"] > 0).float())
                    )
                    > 0.2
                ) * brain_tissue
                brain_tissue[brain_tissue < 0] = 0
                brain_tissue = brain_tissue / brain_tissue.sum(0)  # normalize to [0,1]
                brain_tissue = torch.nan_to_num(
                    brain_tissue, nan=0.0, posinf=0.0, neginf=0.0
                )
                brain_tissue = torch.cat(
                    [1.0 - brain_tissue.sum(0, keepdim=True), brain_tissue], 0
                )
                d[self.new_keys[i]] = paste_lesion(brain_tissue, lesion)
                if key + "_meta_dict" in list(d.keys()):
                    d[self.new_keys[i] + "_meta_dict"] = deepcopy(d[key + "_meta_dict"])
